...
export default function SearchClient({ initialProducts }: SearchClientProps) {
    ...
    // Filter categories and brands from initial products
    ...
    const brands = useMemo(() => {
        const bnds = initialProducts.map(p => p.brand);
        return Array.from(new Set(bnds)).filter(Boolean).sort();
    }, [initialProducts]);
    ...
    const filteredProducts = useMemo(() => {
        ...
        return result;
    }, [initialProducts, searchQuery, selectedCategory, filterParam, selectedBrand, priceRange, showInStockOnly, sortBy]);
